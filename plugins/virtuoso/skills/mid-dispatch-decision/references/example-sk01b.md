> **Illustrative example.** Every identifier, threshold, sample size, and command below
> belongs to the fictional project this walkthrough describes. They are *not* defaults of
> this plugin. A real run resolves standing-rule identifiers from `policy.standingRules`,
> thresholds from the project's own policy, and commands from its registered commands.
...
pre-authorized threshold, and the implementation agent has a hypothesis about root cause. This isn't
a rubber-stamp confirmation.

### Step 1 — Context loaded (Deep mode):
- Pause report: quick-cal Immediate +3.5pp, Special -5.1pp, both exceed 3pp threshold
- Dispatch spec (roadmap SK-01B): "Escalate to full-cal the full sweep if shift
  exceeds 3pp on any primary metric (per RULE-113)"
...
- Lessons learned: RULE-015 (the quick sweep unreliable for structural changes), RULE-113
...
- **Known pattern:** RULE-015 warns that the quick sweep is unreliable for structural changes.
  This is the mid sweep (better) but still single-seed. RULE-113 pre-authorizes full-cal
...
- Dispatch pre-authorizes full-cal at >3pp? **Yes** (per RULE-113).
...
- Alternative frame test: Type 3 (pivot to investigate CHAIN_ENGAGEMENT_THRESHOLD)
  is tempting because the implementation agent identified a specific mechanism. But the investigation
...
The implementation agent's quick-cal found that the Immediate-outcome rate rose 3.5 percentage points
...
Run the full calibration (the full sweep x 3 seeds, ~25 additional minutes). This
...
The implementation agent identified the engagement threshold as the likely mechanism for the Special-outcome drop.
...
- **What the dispatch planned for:** The spec explicitly says "Escalate to
  full-cal the full sweep if shift exceeds 3pp on any primary metric (per RULE-113)."
  Both metrics exceed 3pp. This follows the pre-authorized path.
- **What history says:** RULE-015 warns that quick-cal at the quick sweep is unreliable
  for structural changes. We're at the mid sweep (better), but still single-seed.
  RULE-113 established the 3pp escalation threshold as a standing rule.
...
- **What would change my mind:** If the drift were noise rather than the
  expected behavioral change — but the implementation agent's analysis of CAL-079 seed-42's own
...
**6a. instruction block (printed first — user pastes immediately):**
...
Proceed with full-cal the full sweep seeds as pre-authorized. The drift is expected
...
close-out but do not tune thresholds. I'll scope that on the planning side.

**Updated task plan:**
  ✓ 12. socrates: quick-cal the mid sweepx1 — DONE, escalation triggered
  → 12a. socrates: full-cal the full sweep seeds — proceed
...
outweighs 25-min cost. Flag CHAIN_ENGAGEMENT_THRESHOLD as a planner follow-up.

**Why:** Correctness, not regression. Downstream sprints need a validated baseline.
```

**6b. Amend dispatch spec** — the planner edits Roadmap.md SK-01B entry, appending:
...
**Scope change:** None
**Follow-up items:** CHAIN_ENGAGEMENT_THRESHOLD investigation (planner to scope)
```

**6c. Lessons learned** — No new lesson. Decision followed RULE-113 pre-authorization
exactly as designed. System worked. Noted: "Amended SK-01B spec. No new lesson —
RULE-113 covers this pattern."
