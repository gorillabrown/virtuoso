...
| D23 | `pointer-closeout` reviews every file the dispatch created: temporaries removed, every other file committed, deliberately ignored, or named for a decision *(amended 2026-09-23 at the owner's request)* | `python -m pytest plugins/virtuoso -q -k created_files`; `grep -n "created-files" plugins/virtuoso/skills/pointer-closeout/SKILL.md` | `sprint_guards created-files --base <ref>` lists files added since the base plus untracked files, classifies temporary / untracked / committed, exits 1 while any temporary or unreconciled file remains; the close-out's crossing runs it and the template records the result |
